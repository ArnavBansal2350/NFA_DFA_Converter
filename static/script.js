...
function updateSteps(activeStep) {
  ['dot0','dot1','dot2','dot25','dot3','dot4'].forEach((id, idx) => {
    const dot = document.getElementById(id);
    if (!dot) return;
    dot.classList.remove('active', 'done');
    if (idx < activeStep) dot.classList.add('done');
    else if (idx === activeStep) dot.classList.add('active');
  });
}
...
function showPhase(n) {
  ...
  const stepMap = {0:0, 1:1, 2:2, 25:3, 3:4, 4:5};
...
function renderSteps(steps) {
  const wrapper = document.getElementById('steps_wrapper');
  let html = '';

  for (const step of steps) {
    if (step.type === 'start') {
      html += `
        <div class="step-block step-start">
      ...
      html += `
        <div class="step-block step-process">
          <div class="step-header">
            <span class="step-icon">⚙️</span>
            <span class="step-title">Processing DFA state: <code>${step.current}</code></span>
          </div>`;

      for (const t of step.transitions) {
        html += `
          <div class="step-transition">
            <div class="step-sym">On symbol <code>${t.symbol}</code>:</div>
            <div class="step-parts">
              ${t.parts.map(p => `<div class="step-part">→ ${p}</div>`).join('')}
            </div>
            <div class="step-result ${t.is_new ? 'step-new' : ''}">
              ∴ Result = <strong>${t.result}</strong>
              ${t.is_new ? '<span class="new-badge">NEW STATE!</span>' : ''}
            </div>
          </div>`;
      }
      html += `</div>`;

    } else if (step.type === 'finals') {
      html += `
...
function resetAll() {
  ...
  document.getElementById('steps_wrapper').innerHTML = '';
